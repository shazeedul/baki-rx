...
export const terminalQueries = {
  async getTerminalByPhoneAndStore(phone: string, storeId: string): Promise<TerminalRow | null> {
    ...
    return db.getFirstAsync<TerminalRow>(
      `SELECT id, store_id, tenant_id, store_name, branch_name, pin_hash, jwt_cache, created_at 
       FROM terminals 
  ...
  async upsertTerminals(terminals: TerminalRow[]): Promise<void> {
    ...
    for (const t of terminals) {
      await db.runAsync(
        `INSERT OR REPLACE INTO terminals (id, store_id, tenant_id, store_name, branch_name, pin_hash, jwt_cache, created_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?);`,
        [t.id, t.store_id, t.tenant_id, t.store_name, t.branch_name, t.pin_hash, t.jwt_cache ?? null, t.created_at]
      );
    }
  },
