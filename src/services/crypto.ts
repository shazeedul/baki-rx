...
export const LOCAL_CRYPT_SALT = process.env.EXPO_PUBLIC_LOCAL_CRYPT_SALT || 'baki-rx-secure-salt-value-12938102';
...
export function verifyPin(pin: string, storedPinHash: string): boolean {
  // 1. Check bcrypt if the stored pin_hash starts with a standard bcrypt prefix ($2a$, $2b$, $2y$, $2x$)
  if (storedPinHash && (storedPinHash.startsWith('$2a$') || storedPinHash.startsWith('$2b$') || storedPinHash.startsWith('$2y$') || storedPinHash.startsWith('$2x$'))) {
    try {
      return bcrypt.compareSync(pin, storedPinHash);
    } catch (e) {
      console.warn('Bcrypt verification failed:', e);
    }
  }

  // 2. Fall back to simple SHA256 of (pin + LOCAL_CRYPT_SALT)
  const inputShaHash = simpleSHA256(pin + LOCAL_CRYPT_SALT);
  if (inputShaHash === storedPinHash) {
    return true;
  }

  // 3. Fall back to direct string match (for local test mock hashes)
